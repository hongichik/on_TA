...
window.SheetClient = (function () {
  ...
  async function fetchQuestions(opts) {
    ...
    const url = getUrl();
    if (!url) return null;
    const res = await fetch(url + '?action=questions&bank=' + BANK, { cache: 'no-store' });
  ...
  async function fetchWrongStats(opts) {
    ...
    const url = getUrl();
    if (!url) return null;
    const res = await fetch(url + '?action=wrongstats&bank=' + BANK, { cache: 'no-store' });
    if (!res.ok) throw new Error('HTTP ' + res.status);
    const json = await res.json();
    if (!Array.isArray(json)) throw new Error('Dữ liệu trả về không hợp lệ');
    setWrongCache(json);
  ...
  function reportAnswer(id, correct) {
    ...
    fetch(url, {
      ...
      body: JSON.stringify({ action: 'report', bank: BANK, id: id, correct: !!correct }),
    }).catch(function () {
